...
{
    [HelpURL(
        "https://www.notion.so/PluginYG-d457b23eee604b7aa6076116aab647ed#178130fecabe4b3f81118dfe0fd88ccf"
    )]
    public class ReviewYG : MonoBehaviour
    {
        [Tooltip("Открывать окно авторизации, если пользователь не авторизован.")]
        public enum ForUnauthorized
        {
            OpenAuthDialog,
            ReviewNotAvailable,
            Ignore
        ...
        public void UpdateData()
        {
            ...
            if (
                !showOnMobileDevice
                && (YandexGame.EnvironmentData.isMobile || YandexGame.EnvironmentData.isTablet)
            )
            {
                YandexGame.EnvironmentData.reviewCanShow = false;
            }

            if (forUnauthorized == ForUnauthorized.ReviewNotAvailable && !YandexGame.auth)
            {
                ReviewNotAvailable.Invoke();
                return;
            }
