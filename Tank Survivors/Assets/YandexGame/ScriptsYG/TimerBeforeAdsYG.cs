...
public class TimerBeforeAdsYG : MonoBehaviour
{
    [
        SerializeField,
        Tooltip(
            "Объект таймера перед показом рекламы. Он будет активироваться и деактивироваться в нужное время."
        )
    ]
    ...
    private IEnumerator TimerAdShow()
    {
        while (true)
        {
            ...
            if (objSecCounter == secondObjects.Length)
            {
                ...
                _ = StartCoroutine(BackupTimerClosure());
